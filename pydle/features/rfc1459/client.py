...
class RFC1459Support(BasicClient):
    ...
    def _parse_and_sync_user(self, data):
        if data is None:
            return None

        nickname, username, hostname = self._parse_user(data)
    ...
    def on_raw_invite(self, message):
        ...
        if self.is_same_nick(self.user.nickname, target):
            self.on_invite(self._get_or_create_channel(channel), user)
    ...
    def on_raw_kick(self, message):
        """ KICK command. """
        kicker = self._parse_and_sync_user(message.source)
        ...
        for channel, target_user in itertools.product(channels, targets):
            target = self._parse_and_sync_user(target_user)
            ...
            self.on_kick(self._get_or_create_channel(channel), target, kicker, reason)

    def on_raw_kill(self, message):
        """ KILL command. """
        by = self._parse_and_sync_user(message.source)
        target = self._parse_and_sync_user(message.params[0])
    ...
    def on_raw_mode(self, message):
        ...
        if self.is_channel(target):
            ...
            self._parse_and_sync_user(target)
    ...
    def on_raw_topic(self, message):
        """ TOPIC command. """
        setter = self._parse_and_sync_user(message.source)
    ...
    def on_raw_353(self, message):
        ...
        for entry in names.split():
            ...
            user = self._parse_and_sync_user(safe_entry)
